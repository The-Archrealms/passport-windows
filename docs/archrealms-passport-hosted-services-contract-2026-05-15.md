...
- When production API and AI gateway URLs are configured, the readiness gate calls `/ops/runtime/status`, `/ops/operator/status`, `/ops/storage/status`, `/ai/runtime/status`, and `/ai/runtime/probe` and requires those endpoints to return the expected ready/authorized results. The AI runtime status and probe must also match the approved production environment values for model ID, model artifact hash, license approval, vector store, and knowledge approval root.
