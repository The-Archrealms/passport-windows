...
if ((-not (Test-Path -LiteralPath $readmePath -PathType Leaf)) -or $Force) {
    @(
        ...
        ''
        '## Evidence References To Capture'
        ''
        'Each scenario `evidence_reference` must point to a controlled evidence ID, signed record, export hash, issue ID, or URI captured during the pilot. Supporting dry-run reports may be referenced, but they do not replace participant observation and signoff.'
        ''
        ...
        ''
        '## Required Signoff Boundary'
        ''
        'The packet remains invalid until a real staff/steward participant signs the participant signoff, confirms the Crown-owned device boundary, confirms synthetic/fake balances, confirms no citizen production tokens or production records were used, and records the controlled approval/signature reference.'
        ''
