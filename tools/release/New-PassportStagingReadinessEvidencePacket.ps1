...
if ((-not (Test-Path -LiteralPath $readmePath -PathType Leaf)) -or $Force) {
    @(
        ...
        ''
        'This packet is intentionally invalid until real staging execution evidence, rollback evidence, and signed promotion approvals are recorded. Do not mark boolean confirmations true or replace placeholders until the evidence exists in the controlled staging evidence system.'
        ''
        '## Evidence Files'
        ''
        '| File | Purpose | Must prove |'
        '|---|---|---|'
        '| `staging-operational-drill-report.json` | Non-synthetic staging drill execution. | Production-candidate package upgrade, endpoint failover, signing verification, ledger export replay, recovery/revocation, storage proof, storage redemption dry run, conversion disclosure dry run, telemetry/privacy, incident response, support access controls, AI gateway authentication/privacy, and prohibited monetary claim blocking all ran in staging and passed. |'
        '| `staging-rollback-drill-report.json` | Staging rollback exercise. | New operations were disabled or routed, ledger events were preserved, no deletion/mutation/backdating occurred, pending escrow was resolved by policy, export access stayed available, and production records were untouched. |'
        '| `staging-promotion-approval-record.json` | Approval to promote out of staging. | Product, engineering, security/privacy, and Crown monetary authority approvals are signed and reference the exact pre-MVP report, staging artifact validation report, operational drill hash, and rollback drill hash. |'
        ''
        '## Required Evidence References'
        ''
        'The operational drill must include at least three controlled evidence references. Prefer references that cover these groups:'
        ''
        '- Package upgrade and artifact validation evidence.'
        '- Endpoint, signing, AI gateway, and telemetry evidence.'
        '- Ledger export replay, recovery/revocation, storage proof, storage redemption, and conversion-disclosure evidence.'
        ''
        'The rollback drill must record at least two approvers, at least one affected service class, and at least one affected asset. The promotion approval must hash-match the operational and rollback files generated in this packet.'
        ''
