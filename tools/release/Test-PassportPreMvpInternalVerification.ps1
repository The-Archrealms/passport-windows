...
$requirements = @(
    ...
    New-Requirement -Id "token_ready_mvp_completion_audit_report" -Description "Operators can generate and validate a prompt-to-artifact completion audit mapping PRD success criteria and ARD release gates to concrete evidence and blockers." -CheckIds @("token_ready_mvp_completion_audit_report_validation") -Checks $checks -Evidence "Completion audit validation generates the current report, verifies source evidence files, checks required checklist IDs, validates status counts, and confirms completion_ready consistency."
