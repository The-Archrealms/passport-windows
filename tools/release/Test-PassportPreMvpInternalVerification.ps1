...
if ($SkipDotnetTests) {
    ...
    $checks += New-Check `
        -Id "storage_redemption_targeted_tests" `
        -Description "Targeted storage-redemption tests cover quote acceptance, escrow, proof-linked burns, refund, re-credit, extension, and admin override paths." `
        -Passed $false `
        -Failures @("Targeted storage-redemption tests were skipped; pre-MVP verification cannot pass with skipped tests.")
    $checks += New-Check `
        -Id "core_monetary_protocol_targeted_tests" `
        -Description "Targeted core monetary tests cover ARCH genesis limits, CC escrow/burn/refund/re-credit semantics, conversion policy inspection, capacity policy inspection, and export verification." `
        -Passed $false `
        -Failures @("Targeted core monetary protocol tests were skipped; pre-MVP verification cannot pass with skipped tests.")
    $checks += New-Check `
        -Id "windows_monetary_ledger_targeted_tests" `
        -Description "Targeted Windows monetary ledger tests cover production wallet signatures, ARCH genesis evidence, CC capacity evidence, revoked wallets, conversion execution, and export packages." `
        -Passed $false `
        -Failures @("Targeted Windows monetary ledger tests were skipped; pre-MVP verification cannot pass with skipped tests.")
    $checks += New-Check `
        -Id "hosted_ai_targeted_tests" `
        -Description "Targeted hosted AI tests cover authenticated sessions, no bearer-token storage, quotas, non-authority boundaries, approved knowledge retrieval, runtime readiness, and runtime probes." `
        -Passed $false `
        -Failures @("Targeted hosted AI tests were skipped; pre-MVP verification cannot pass with skipped tests.")
    $checks += New-Check `
        -Id "windows_ai_gateway_targeted_tests" `
        -Description "Targeted Windows AI gateway tests cover signed session requests, remote hosted sessions, revoked AI sessions, tamper rejection, approved knowledge answers, and secret blocking." `
        -Passed $false `
        -Failures @("Targeted Windows AI gateway tests were skipped; pre-MVP verification cannot pass with skipped tests.")
}
else {
    ...
    $checks += New-ToolCheck `
        -Id "storage_redemption_targeted_tests" `
        -Description "Targeted storage-redemption tests cover quote acceptance, escrow, proof-linked burns, refund, re-credit, extension, and admin override paths." `
        -Result (Invoke-Tool -FilePath $dotnet -Arguments @("test", "tests\ArchrealmsPassport.Windows.Tests\ArchrealmsPassport.Windows.Tests.csproj", "-c", $Configuration, "--no-restore", "--filter", "FullyQualifiedName~PassportStorageRedemptionServiceTests"))
    $checks += New-ToolCheck `
        -Id "core_monetary_protocol_targeted_tests" `
        -Description "Targeted core monetary tests cover ARCH genesis limits, CC escrow/burn/refund/re-credit semantics, conversion policy inspection, capacity policy inspection, and export verification." `
        -Result (Invoke-Tool -FilePath $dotnet -Arguments @("test", "tests\ArchrealmsPassport.Core.Tests\ArchrealmsPassport.Core.Tests.csproj", "-c", $Configuration, "--no-restore", "--filter", "FullyQualifiedName~PassportMonetaryLedgerSemanticsTests|FullyQualifiedName~PassportMonetaryProtocolTests|FullyQualifiedName~PassportMonetaryLedgerExportVerifierTests|FullyQualifiedName~PassportRegistryRecordInspectorTests"))
    $checks += New-ToolCheck `
        -Id "windows_monetary_ledger_targeted_tests" `
        -Description "Targeted Windows monetary ledger tests cover production wallet signatures, ARCH genesis evidence, CC capacity evidence, revoked wallets, conversion execution, and export packages." `
        -Result (Invoke-Tool -FilePath $dotnet -Arguments @("test", "tests\ArchrealmsPassport.Windows.Tests\ArchrealmsPassport.Windows.Tests.csproj", "-c", $Configuration, "--no-restore", "--filter", "FullyQualifiedName~PassportMonetaryLedgerServiceTests|FullyQualifiedName~PassportCrownCreditCapacityServiceTests|FullyQualifiedName~PassportConversionQuoteServiceTests"))
    $checks += New-ToolCheck `
        -Id "hosted_ai_targeted_tests" `
        -Description "Targeted hosted AI tests cover authenticated sessions, no bearer-token storage, quotas, non-authority boundaries, approved knowledge retrieval, runtime readiness, and runtime probes." `
        -Result (Invoke-Tool -FilePath $dotnet -Arguments @("test", "tests\ArchrealmsPassport.HostedServices.Tests\ArchrealmsPassport.HostedServices.Tests.csproj", "-c", $Configuration, "--no-restore", "--filter", "FullyQualifiedName~PassportHostedAiRuntimeTests|FullyQualifiedName~PassportHostedPolicyTests"))
    $checks += New-ToolCheck `
        -Id "windows_ai_gateway_targeted_tests" `
        -Description "Targeted Windows AI gateway tests cover signed session requests, remote hosted sessions, revoked AI sessions, tamper rejection, approved knowledge answers, and secret blocking." `
        -Result (Invoke-Tool -FilePath $dotnet -Arguments @("test", "tests\ArchrealmsPassport.Windows.Tests\ArchrealmsPassport.Windows.Tests.csproj", "-c", $Configuration, "--no-restore", "--filter", "FullyQualifiedName~PassportAiGatewayServiceTests"))
}
...
if ($SkipDeploymentValidation) {
    ...
    $checks += New-Check `
        ...
        -Description "ProductionMvp next-action packet generation validates the operator handoff for remaining blockers." `
        -Passed $false `
        -Failures @("ProductionMvp next-action packet validation was skipped; pre-MVP verification cannot pass with skipped deployment validation.")
    $checks += New-Check `
        ...
        -Passed $false `
        -Failures @("Staff/steward pilot handoff validation was skipped; pre-MVP verification cannot pass with skipped deployment validation.")
    $checks += New-Check `
        -Id "staff_steward_pilot_dry_run_validation" `
        -Description "Staff/steward pilot dry-run evidence generation validates scenario references and boundary flags before the real pilot evidence packet is signed." `
        -Passed $false `
        -Failures @("Staff/steward pilot dry-run validation was skipped; pre-MVP verification cannot pass with skipped deployment validation.")
    $checks += New-Check `
    ...
    $checks += New-Check `
        -Id "staging_readiness_evidence_packet_validation" `
        -Description "Staging readiness evidence packet templates validate operational drill, rollback drill, and promotion approval contracts." `
        -Passed $false `
        -Failures @("Staging readiness evidence packet validation was skipped; pre-MVP verification cannot pass with skipped deployment validation.")
    $checks += New-Check `
        -Id "staging_readiness_closeout_validation" `
        -Description "Staging readiness closeout validates a filled staging packet and produces the report/hash inputs required by canary and production." `
        -Passed $false `
        -Failures @("Staging readiness closeout validation was skipped; pre-MVP verification cannot pass with skipped deployment validation.")
    $checks += New-Check `
    ...
    $checks += New-Check `
        -Id "canary_mvp_readiness_evidence_packet_validation" `
        -Description "Canary MVP readiness evidence packet templates validate policy, incident, balance, service-delivery, support, and production approval contracts." `
        -Passed $false `
        -Failures @("Canary MVP readiness evidence packet validation was skipped; pre-MVP verification cannot pass with skipped deployment validation.")
    $checks += New-Check `
        -Id "canary_mvp_readiness_closeout_validation" `
        -Description "Canary MVP readiness closeout validates a filled canary packet and produces the report/hash inputs required by ProductionMvp readiness." `
        -Passed $false `
        -Failures @("Canary MVP readiness closeout validation was skipped; pre-MVP verification cannot pass with skipped deployment validation.")
    $checks += New-Check `
    ...
    $checks += New-Check `
        ...
        -Description "Production MVP closeout generates a validated outstanding-work and next-action handoff when closeout fails." `
        -Passed $false `
        -Failures @("Production MVP closeout failure-handoff validation was skipped; pre-MVP verification cannot pass with skipped deployment validation.")
}
else {
    $checks += New-ToolCheck `
        -Id "package_signing_provisioning_validation" `
    ...
    $checks += New-ToolCheck `
        -Id "production_mvp_closeout_validation" `
        -Description "Production MVP closeout validates provisioning, readiness, and release evidence orchestration before final production packaging." `
        -Result (Invoke-Tool -FilePath "powershell" -Arguments @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "tools\release\Complete-PassportProductionMvpCloseout.ps1", "-UseGeneratedFixture"))
    $checks += New-ToolCheck `
        -Id "production_mvp_closeout_failure_handoff_validation" `
    ...
    $checks += New-ToolCheck `
        ...
        -Result (Invoke-Tool -FilePath "powershell" -Arguments @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "tools\release\Test-PassportTokenReadyMvpCompletionAuditReport.ps1", "-UseGeneratedFixture", "-Generate", "-NoFail"))
    $checks += New-ToolCheck `
    ...
    $checks += New-ToolCheck `
        -Id "staff_steward_pilot_handoff_validation" `
        -Description "Staff/steward pilot handoff generation and fail-closed validation are exercised before the external pilot evidence gate can be closed." `
        -Result (Invoke-Tool -FilePath "powershell" -Arguments @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "tools\release\Test-PassportPreMvpStaffStewardPilotHandoff.ps1", "-UseGeneratedFixture", "-OutputPath", "artifacts\release\pre-mvp-staff-steward-pilot-handoff-validation-report.json"))
    $checks += New-ToolCheck `
        -Id "staff_steward_pilot_dry_run_validation" `
        -Description "Staff/steward pilot dry-run evidence generation validates scenario references and boundary flags before the real pilot evidence packet is signed." `
        -Result (Invoke-Tool -FilePath "powershell" -Arguments @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "tools\release\Test-PassportPreMvpStaffStewardPilotDryRunEvidence.ps1", "-UseGeneratedFixture", "-OutputPath", "artifacts\release\pre-mvp-staff-steward-pilot-dry-run-validation-report.json"))
    $checks += New-ToolCheck `
        -Id "staff_steward_pilot_report_validation" `
        -Description "Staff/steward pilot report validation is exercised before the external pilot evidence gate can be closed." `
    ...
    $checks += New-ToolCheck `
        -Id "staging_readiness_evidence_packet_validation" `
        -Description "Staging readiness evidence packet templates validate operational drill, rollback drill, and promotion approval contracts." `
        -Result (Invoke-Tool -FilePath "powershell" -Arguments @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "tools\release\Test-PassportStagingReadinessEvidencePacket.ps1", "-OutputPath", "artifacts\release\staging-readiness-evidence-template-validation-report.json"))
    $checks += New-ToolCheck `
        -Id "staging_readiness_evidence_fill_helper_validation" `
        -Description "Staging readiness evidence fill helper generates a completed packet with explicit confirmations and validates hashes before operator closeout." `
        -Result (Invoke-Tool -FilePath "powershell" -Arguments @(
            "-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "tools\release\Set-PassportStagingReadinessEvidencePacket.ps1",
            "-PacketRoot", "artifacts\release\staging-readiness-evidence-fill-validation\packet",
            "-EngineeringSignoffId", "engineering-signoff-validation",
            "-SecurityPrivacySignoffId", "security-privacy-signoff-validation",
            "-CrownMonetaryAuthoritySignoffId", "crown-monetary-authority-signoff-validation",
            "-ApiBaseUrl", "https://staging-api.validation.archrealms.invalid",
            "-AiGatewayUrl", "https://staging-ai.validation.archrealms.invalid",
            "-TelemetryDestination", "staging-telemetry-validation",
            "-PackageVersion", "staging-package-validation",
            "-Operator", "staging-operator-validation",
            "-IncidentResponseOwner", "staging-incident-owner-validation",
            "-OperationalEvidenceReference", "evidence://validation/upgrade;evidence://validation/failover;evidence://validation/ledger",
            "-RollbackReasonCode", "staging-validation-rollback-drill",
            "-RollbackApprover", "engineering-approver-validation;security-privacy-approver-validation",
            "-RollbackUserFacingStatus", "Staging rollback validation status",
            "-ConfirmOperationalDrillCompleted",
            "-ConfirmProductionCandidateUpgradeValidated",
            "-ConfirmEndpointFailoverValidated",
            "-ConfirmSigningVerificationValidated",
            "-ConfirmLedgerExportReplayValidated",
            "-ConfirmRecoveryRevocationValidated",
            "-ConfirmStorageProofValidationCompleted",
            "-ConfirmStorageRedemptionDryRunCompleted",
            "-ConfirmConversionDisclosureDryRunCompleted",
            "-ConfirmTelemetryPrivacyValidated",
            "-ConfirmIncidentResponseValidated",
            "-ConfirmSupportAccessControlsValidated",
            "-ConfirmAiGatewayAuthPrivacyValidated",
            "-ConfirmProhibitedClaimsBlocked",
            "-ConfirmRollbackDrillCompleted",
            "-ConfirmNewOperationsDisabledOrRouted",
            "-ConfirmLedgerEventsPreserved",
            "-ConfirmNoDeletionMutationOrBackdating",
            "-ConfirmPendingEscrowResolvedByPolicy",
            "-ConfirmExportAccessPreserved",
            "-ConfirmProductionRecordsUntouched",
            "-ConfirmCanaryOrProductionReleaseApproved",
            "-ConfirmProductApprovalSigned",
            "-ConfirmEngineeringSignoffSigned",
            "-ConfirmSecurityPrivacySignoffSigned",
            "-ConfirmCrownMonetaryAuthoritySignoffSigned",
            "-Force"
        ))
    $checks += New-ToolCheck `
        -Id "staging_readiness_closeout_validation" `
        -Description "Staging readiness closeout validates a filled staging packet and produces the report/hash inputs required by canary and production." `
        -Result (Invoke-Tool -FilePath "powershell" -Arguments @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "tools\release\Complete-PassportStagingReadinessEvidencePacket.ps1", "-UseGeneratedFixture"))
    $checks += New-ToolCheck `
        -Id "canary_mvp_readiness_gate_validation" `
        -Description "Canary MVP readiness gate generation and validation are exercised before ProductionMvp release gating can depend on it." `
    ...
    $checks += New-ToolCheck `
        -Id "canary_mvp_readiness_evidence_packet_validation" `
        -Description "Canary MVP readiness evidence packet templates validate policy, incident, balance, service-delivery, support, and production approval contracts." `
        -Result (Invoke-Tool -FilePath "powershell" -Arguments @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "tools\release\Test-PassportCanaryMvpReadinessEvidencePacket.ps1", "-OutputPath", "artifacts\release\canary-mvp-readiness-evidence-template-validation-report.json"))
    $checks += New-ToolCheck `
        -Id "canary_mvp_readiness_evidence_fill_helper_validation" `
        -Description "Canary MVP readiness evidence fill helper generates a completed packet with explicit confirmations and validates hashes before operator closeout." `
        -Result (Invoke-Tool -FilePath "powershell" -Arguments @(
            "-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "tools\release\Set-PassportCanaryMvpReadinessEvidencePacket.ps1",
            "-PacketRoot", "artifacts\release\canary-mvp-readiness-evidence-fill-validation\packet",
            "-SupportOwner", "canary-support-owner-validation",
            "-IncidentResponseOwner", "canary-incident-owner-validation",
            "-RollbackPolicyId", "canary-rollback-policy-validation",
            "-EngineeringSignoffId", "engineering-signoff-validation",
            "-SecurityPrivacySignoffId", "security-privacy-signoff-validation",
            "-CrownMonetaryAuthoritySignoffId", "crown-monetary-authority-signoff-validation",
            "-PolicyEvidenceReference", "evidence://validation/canary-policy",
            "-IncidentEvidenceReference", "evidence://validation/canary-incident",
            "-BalanceEvidenceReference", "evidence://validation/canary-balance",
            "-ServiceDeliveryEvidenceReference", "evidence://validation/canary-service-delivery",
            "-SupportEvidenceReference", "evidence://validation/canary-support",
            "-ApprovalNotes", "Canary production approval validation notes",
            "-ConfirmProductionIntended",
            "-ConfirmProductionLedger",
            "-ConfirmAllowlistedCitizensOnly",
            "-ConfirmIncidentReviewCompleted",
            "-ConfirmNoUnresolvedCriticalIncidents",
            "-ConfirmNoUnresolvedHighIncidents",
            "-ConfirmBalancesReconciled",
            "-ConfirmEscrowReconciled",
            "-ConfirmBurnRefundRecreditReconciled",
            "-ConfirmCrownReserveReconciled",
            "-ConfirmNoNegativeBalances",
            "-ConfirmNoUnapprovedIssuance",
            "-ConfirmNoStagingRecordsDetected",
            "-ConfirmServiceDeliveryReconciled",
            "-ConfirmStorageRedemptionsReconciled",
            "-ConfirmStorageProofsReconciled",
            "-ConfirmBurnsMatchVerifiedEpochs",
            "-ConfirmRefundsRecreditsExtensionsReconciled",
            "-ConfirmSupportReady",
            "-ConfirmSupportQueueReviewed",
            "-ConfirmRecoverySupportReady",
            "-ConfirmEscalationPathReady",
            "-ConfirmSupportAccessControlsValidated",
            "-ConfirmProductionMvpReleaseApproved",
            "-Force"
        ))
    $checks += New-ToolCheck `
        -Id "canary_mvp_readiness_closeout_validation" `
...
$requirements = @(
    ...
    New-Requirement -Id "staging_readiness_evidence_packet" -Description "The staging readiness evidence packet can be generated, filled, and validated before staging readiness values are loaded." -CheckIds @("staging_readiness_evidence_packet_validation", "staging_readiness_evidence_fill_helper_validation") -Checks $checks -Evidence "Staging readiness evidence packet validation checks operational drill, rollback drill, promotion approval, hashes, and cross-references; the fill helper requires explicit operator confirmations and recomputes source report hashes."
    New-Requirement -Id "staging_readiness_closeout" -Description "The filled staging readiness packet has a single fail-closed closeout path before canary or production readiness can depend on it." -CheckIds @("staging_readiness_closeout_validation") -Checks $checks -Evidence "Staging closeout validation verifies a filled packet, generates a closeout environment, runs staging readiness, and emits the staging readiness report path and SHA-256 needed downstream."
    ...
    New-Requirement -Id "canary_mvp_readiness_evidence_packet" -Description "The Canary MVP readiness evidence packet can be generated, filled, and validated before canary readiness values are loaded." -CheckIds @("canary_mvp_readiness_evidence_packet_validation", "canary_mvp_readiness_evidence_fill_helper_validation") -Checks $checks -Evidence "Canary readiness evidence packet validation checks policy limits, incident review, balance reconciliation, service delivery reconciliation, support readiness, production approval, hashes, and cross-references; the fill helper requires explicit operator confirmations and recomputes source report hashes."
    New-Requirement -Id "canary_mvp_readiness_closeout" -Description "The filled Canary MVP readiness packet has a single fail-closed closeout path before ProductionMvp readiness can depend on it." -CheckIds @("canary_mvp_readiness_closeout_validation") -Checks $checks -Evidence "Canary closeout validation verifies a filled packet, generates a closeout environment, runs canary readiness, and emits the canary readiness report path and SHA-256 needed downstream."
    ...
    New-Requirement -Id "no_fake_record_migration" -Description "Pre-MVP fake/synthetic records cannot migrate into production ARCH, CC, Crown reserve, citizen account, or production service-liability records." -CheckIds @("core_monetary_protocol_targeted_tests", "windows_monetary_ledger_targeted_tests", "internal_verification_artifact_lane") -Checks $checks -Evidence "Release-lane artifact validation and ledger replay enforce non-production lane isolation."
)
