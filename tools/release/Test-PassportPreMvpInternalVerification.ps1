...
$requirements = @(
    ...
    New-Requirement -Id "production_mvp_closeout" -Description "The filled production provisioning packet and ProductionMvp readiness report have a single fail-closed closeout path before production packaging." -CheckIds @("production_mvp_closeout_validation") -Checks $checks -Evidence "Production closeout validation exercises the final orchestration that validates provisioning, verifies readiness, generates redacted release evidence, and requires release evidence to be ready before packaging."
