...
if ($SkipDeploymentValidation) {
    ...
    $checks += New-ToolCheck `
        ...
        -Result (Invoke-Tool -FilePath "powershell" -Arguments @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "tools\release\New-PassportProductionMvpOutstandingWorkReport.ps1", "-UseGeneratedFixture"))
    $checks += New-ToolCheck `
        -Id "token_ready_mvp_completion_audit_report_validation" `
        -Description "Token-Ready MVP completion audit maps PRD success criteria and ARD release gates to concrete evidence files." `
        -Result (Invoke-Tool -FilePath "powershell" -Arguments @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "tools\release\Test-PassportTokenReadyMvpCompletionAuditReport.ps1", "-Generate", "-NoFail"))
    $checks += New-ToolCheck `
...
$requirements = @(
    ...
    New-Requirement -Id "production_mvp_outstanding_work_report" -Description "Operators can generate a concise redacted report of remaining ProductionMvp closeout, readiness, provisioning, and release-evidence blockers." -CheckIds @("production_mvp_outstanding_work_report_validation") -Checks $checks -Evidence "Outstanding-work report generation uses synthetic fixtures to verify blocker grouping, operator actions, and secret-free JSON/Markdown output."
    New-Requirement -Id "token_ready_mvp_completion_audit_report" -Description "Operators can generate and validate a prompt-to-artifact completion audit mapping PRD success criteria and ARD release gates to concrete evidence and blockers." -CheckIds @("token_ready_mvp_completion_audit_report_validation", "token_ready_mvp_completion_audit_complete_fixture_validation") -Checks $checks -Evidence "Completion audit validation generates the current report, verifies source evidence files, checks required checklist IDs, validates status counts, confirms completion_ready consistency, validates non-passed operator action commands against real release scripts, verifies Markdown Next/Command coverage, and proves RequireComplete against a generated complete-state fixture."
