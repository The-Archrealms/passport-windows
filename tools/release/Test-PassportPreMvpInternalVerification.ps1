...
if ($SkipDeploymentValidation) {
    ...
    $checks += New-ToolCheck `
        ...
        -Result (Invoke-Tool -FilePath "powershell" -Arguments @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "tools\release\New-PassportTokenReadyMvpCompletionAuditReport.ps1", "-NoFail"))
    $checks += New-ToolCheck `
        -Id "staff_steward_pilot_handoff_validation" `
        -Description "Staff/steward pilot handoff generation and fail-closed validation are exercised before the external pilot evidence gate can be closed." `
...
$requirements = @(
    ...
    New-Requirement -Id "token_ready_mvp_completion_audit_report" -Description "Operators can generate a prompt-to-artifact completion audit mapping PRD success criteria and ARD release gates to concrete evidence and blockers." -CheckIds @("token_ready_mvp_completion_audit_report_validation") -Checks $checks -Evidence "Completion audit generation reads current pre-MVP, staging, canary, production readiness, closeout, and outstanding-work reports and emits a checklist that separates passed, partial, blocked, and missing requirements."
